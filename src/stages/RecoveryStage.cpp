...
void timerRecovery() {
  int deployTime = EEPROM.read(ADDRESS_TIMER_FLIGHT_MODE_DEPLOY_TIME_MS);
  int servo2DeployTime = EEPROM.read(ADDRESS_SERVO_2_DEPLOY_TIME_MS);
  ...
  if (t1 >= (timeThreshold)) {                      
    ...
    if (timer == true && t1 >= (timeThreshold + SERVO_POWER_SAVING_TIMEOUT_MS)) {  //We disable the servo after 2 seconds to save power
      servo1.detach();
    }
